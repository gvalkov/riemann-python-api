...
Tool for quering or sending events to Riemann. Example usage:
  ...
  %(prog)s tcp://127.0.0.1:5555 insert service=A tags=a,b,c metric=100
  %(prog)s udp://127.0.0.1:5555 insert "{'service': 'B', 'metric': 100}"

Connection string:
  The location of the Riemann server may be set with the first positional
  argument to the script. For example:

    %(prog) tcp://localhost:5555 insert service=A metric=100

  The accepted format is 'scheme://host:port' where 'scheme' and 'port'
  are optional and default to 'tcp' and '5555'. The available connection
  schemes are: tcp udp tls

  If a connection string is not set, the tool will attempt to connect to
  'tcp://localhost:5555' or the value of the RIEMANN_TOOL_URL environment
  variable.

Environment variables:
  The following environment variables are understood by this this tool:

  RIEMANN_TOOL_URL:  Riemann server to connect to.
  RIEMANN_TOOL_TAG:
...
Options:
  ...
  --no-env-overwrites   Ignore all RIEMANN_TOOL_* variables.
...
def parseargs(args):
    ...
    parser.add_option('--no-env-overwrites', action='store_true')
